...
export const getCountry = (countryCOde: string): string | null => {
  if (countryCOde === null) return null;
  let countryElement = countryList.find((element) => {
    if (element.code === countryCOde) return element;
  });
  if (countryElement) return countryElement?.country;
  return null;
};
...
export const getCountryCode = (country: string): string | null => {
  ...
  if (countryElement) return countryElement?.code;
  return null;
};